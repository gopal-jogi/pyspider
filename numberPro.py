...
def spy(num,r=1):
    for val in [int(dig) for dig in str(num)]:
        r*=val
    return 'Spy Number' if sum([int(num) for num in str(num)])==r else 'Not Spy Number'
...
# print( tuple(val for val in [int(dig) for dig in str(453)]))
